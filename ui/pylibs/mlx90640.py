...
class I2CDevice:
    ...
    def read_into(self, buf: bytearray, *, start: int = 0, end: int = None) -> None:
        ...
        if not (0 <= start < end <= len(buf)):
            raise ValueError(
                f"Invalid start/end range: start={start}, end={end}, len={len(buf)}")
    ...
    def write_then_read_into(self, out_buffer: bytes, in_buffer: bytearray, *, out_start: int = 0, out_end: int = None,
        ...
        if not (0 <= out_start <= out_end <= len(out_buffer)):
            raise ValueError(
                f"Invalid out_buffer range: start={out_start}, end={out_end}, len={len(out_buffer)}")
        if not (0 <= in_start <= in_end <= len(in_buffer)):
            raise ValueError(
                f"Invalid in_buffer range: start={in_start}, end={in_end}, len={len(in_buffer)}")

        self.i2c.writeto(self.device_address, memoryview(
            out_buffer)[out_start:out_end], False)
        self.i2c.readfrom_into(self.device_address,
                               memoryview(in_buffer)[in_start:in_end])
    ...
    def _probe_for_device(self) -> None:
        try:
            ...
            try:
                ...
                raise ValueError(
                    f'No I2C device at address: 0x{self.device_address:x}')
...
class MLX90640:
    ...
    def __init__(self, i2c_bus: machine.I2C, address: int = None):
        ...
        if not isinstance(i2c_bus, machine.I2C):
            raise TypeError(
                f"i2c_bus must be a machine.I2C instance, got {type(i2c_bus).__name__}")
        if not isinstance(address, int):
            raise TypeError(
                f"address must be an int or None, got {type(address).__name__}")
    ...
    def _get_ta(self) -> float:
        ...
        if ptat_art > 32767:
            ptat_art -= 65536
        ptat_art = (ptat / (ptat * self.alpha_ptat + ptat_art)) * \
            math.pow(2, 18)
    ...
    def _get_vdd(self) -> float:
        ...
        resolution_correction = math.pow(
            2, self.resolution_ee) / math.pow(2, resolution_ram)
    ...
    def _calculate_to(self, emissivity: float, tr: float, result: list) -> None:
        ...
        alpha_corr_r[3] = alpha_corr_r[2] * \
            (1 + self.ks_to[2] * (self.ct[3] - self.ct[2]))
        ...
        ir_data_cp[0] -= self.cp_offset[0] * \
            (1 + self.cp_kta * (ta - 25)) * (1 + self.cp_kv * (vdd - 3.3))
        if mode == self.calibration_mode_ee:
            ir_data_cp[1] -= self.cp_offset[1] * \
                (1 + self.cp_kta * (ta - 25)) * (1 + self.cp_kv * (vdd - 3.3))
        else:
        ...
        for pixel_number in range(768):
            ...
            if mode == 0:
                ...
                chess_pattern = il_pattern ^ (
                    pixel_number - (pixel_number // 2) * 2)
            ...
            if pattern == sub_page:
                ...
                ir_data -= self.offset[pixel_number] * \
                    (1 + kta * (ta - 25)) * (1 + kv * (vdd - 3.3))

                if mode != self.calibration_mode_ee:
                    ir_data += self.il_chess_c[2] * (
                        2 * il_pattern - 1) - self.il_chess_c[1] * conversion_pattern
                ...
                to = math.sqrt(math.sqrt(
                    (ir_data / (alpha_compensated *
                     (1 - self.ks_to[1] * 273.15) + sx) + ta_tr)
                )) - 273.15
    ...
    def _extract_alpha_parameters(self) -> None:
        ...
        for i in range(24):
            for j in range(32):
                ...
                alpha_temp[p] -= self.tgc * \
                    (self.cp_alpha[0] + self.cp_alpha[1]) / 2
