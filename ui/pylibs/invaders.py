...
#
# Entry points:
#   run(oled, btn, x0=28, y0=24, led=None)  - what the Play Invaders block calls.
#                                             x0/y0 are the window's origin inside
#                                             the 128x64 the display claims to be.
#   demo()                                  - the same thing from the REPL, wired
#                                             for the classroom robot's pins:
...
# hold ~2s = clear the saved high score. Exit: press RST, which reboots the board.
#
...
def _quiet_the_robot():
    """Stop anything that repaints the OLED or shares its I2C bus.

    A game owns the screen until somebody presses RST, and the two things
    that would otherwise keep drawing on it are both the classroom robot's:
    gyro.py samples the MPU-6050 on Timer 1, robot.py drives the motors and
    redraws the dashboard on Timer 0. Each is guarded on its own, so a board
    carrying only one of them still quietens that one, and a board carrying
    neither passes straight through.

    snake.py and defender.py carry the same function. Three copies rather
    than a fourth module, because each game is installed on its own and has
    to run with nothing beside it but ssd1306.py.
    """
    try:
