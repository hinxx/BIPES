# robot.py -- the runtime the Robot blocks call, for a classroom robot built
# on an ESP32-C3 Super Mini (01Space OLED variant).
#
# From setownley/BIPES, firmware/invaders/robot.py at v0.6.2. Install it on the
# board with the "Install robot library" button in the Robot category; every
# block in that category compiles to a one-line call into this module.
#
...
#
# The fork's v0.6.2 also carried a hidden ten-second button hold at the
# calibration gate that handed the board to an arcade menu. That is not here:
# the games it launches are their own backlog item, and a module that silently
# stops being a robot when a button is held too long is a support call, not a
# feature. cal_gate() is the v0.5.8 one -- tap to calibrate, or don't.
...
def _load_breakaway():
    ...
    if _breakaway["A"] == 0 or _breakaway["B"] == 0:
        print("robot: breakaway not calibrated - run bench.run('launch')")
...
# The "Robot OS background timer" block can start a program with the background
# system off, so electronics lessons can drive the ultrasonic, QRE and OLED
# directly without Timer 0 fighting them. In this tree that block emits a call
# to os_timer() at the top of the program -- the fork instead wrote the setting
# to a file before every Run, which needs a deploy pipeline this tree does not
# have.
#
# SETTINGS_FILE is still read at import, for a board that should come up with
# the timer off whatever program runs: put OS_TIMER=0 in it. Missing, unreadable
# or malformed file => True => the timer runs, which is the default.
...
def cal_gate():
    """Teacher utility, for a main.py that should offer calibration at boot:
    3s window: tap = kid self-cal (then halt); no tap = run the program.

    No block calls this and nothing prepends it -- call it from main.py, above
    whatever the board should run."""
    show("TAP=CAL")
