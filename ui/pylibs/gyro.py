...
def gyro_setup(i2c=None, addr=0x68, bus=0, sda=5, scl=6):
    """Wake the gyro, calibrate it, and start sampling in the background.

    i2c is an I2C object to share. The block passes the one every other I2C
    block on the same pins uses, so the gyro, the display and the distance
    sensor are one bus object rather than three initialising the same
    peripheral behind each other's backs.

    Without one, a bus is built from bus/sda/scl -- the classroom robot's
    pins by default, which is what makes gyro_turn() work at the REPL with
