...
| `offered` | `false` defines the block and lists it nowhere -- a superseded block a saved program may still contain. `boards:` and `offered: false` contradict each other. On an `external:` entry it leaves the entry as the record that a hand-written block exists and is deliberately in no flyout (the two RFID placeholders). |
